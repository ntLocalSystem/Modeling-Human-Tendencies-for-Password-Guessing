...
# date : 15-12-2020 (rev 1)
# date : 19-12-2020 (rev 1.1) Fixes
# date : 24-01-2021 (rev 1.2) Changes for state structure
# date : 9-2-2021 (rev 1.3) (Added Length Normalization)
# date : 10-2-2021(rev 1.4) (Added Unlimited Generation and Exit Handling)

# This script is for sampling novel sequences
# with probability greater than threshold.
# This is only applicable to LSTM Networks.
...
class Guesser():
    def __init__(
        ...
        VOCAB_SIZE, LSTM_UNITS, PREDICTION_BATCH_SIZE,
        ...
        # All are 0s
        self.lstm_h_1 = np.zeros((1, self.lstm_units))
        self.lstm_c_1 = np.zeros((1, self.lstm_units))
        self.lstm_h_2 = np.zeros((1, self.lstm_units))
        self.lstm_c_2 = np.zeros((1, self.lstm_units))
        self.lstm_h_3 = np.zeros((1, self.lstm_units))
        self.lstm_c_3 = np.zeros((1, self.lstm_units))

        # Building the Initial Data Structure to be supplied with the prefix
        self.start_state = {
            "1": (self.lstm_h_1, self.lstm_c_1),
            "2": (self.lstm_h_2, self.lstm_c_2),
            "3": (self.lstm_h_3, self.lstm_c_3)
            }
        ...
        self.testing_checkpoint_guesses = TESTING_CHECKPOINT
    ...
    def exit_handler(self):
        print(f"\nTotal passwords generated are: {self.pwd_generated}\n")
    ...
    def complete_guessing(self, start="", start_prob=1.0):
        print(f"[+] Enumerating guesses starting from {repr(start)}")
        # Start Guessing
        try:
    ...
    def calculate_conditional_probs(self, subset_node_list):
        ...
        lstm_1_h_states = np.zeros((0, self.lstm_units))
        lstm_1_c_states = np.zeros((0, self.lstm_units))
        lstm_2_h_states = np.zeros((0, self.lstm_units))
        lstm_2_c_states = np.zeros((0, self.lstm_units))
        lstm_3_h_states = np.zeros((0, self.lstm_units))
        lstm_3_c_states = np.zeros((0, self.lstm_units))

        # Add the states to the placeholders :
        for node in subset_node_list:
            lstm_1_h_states = np.concatenate((lstm_1_h_states, node[2]["1"][0]), 0)
            lstm_1_c_states = np.concatenate((lstm_1_c_states, node[2]["1"][1]), 0)

            lstm_2_h_states = np.concatenate((lstm_2_h_states, node[2]["2"][0]), 0)
            lstm_2_c_states = np.concatenate((lstm_2_c_states, node[2]["2"][1]), 0)

            lstm_3_h_states = np.concatenate((lstm_3_h_states, node[2]["3"][0]), 0)
            lstm_3_c_states = np.concatenate((lstm_3_c_states, node[2]["3"][1]), 0)

        # Add Batch Size here - gpu max size

        predictions = self.model.predict(
            [encoded_batch_prev_chars,
            lstm_1_h_states, lstm_1_c_states,
            lstm_2_h_states, lstm_2_c_states,
            lstm_3_h_states, lstm_3_c_states],
        ...
        # LSTM Hidden State Layer 1 - (self.gpu_prediction_batch_size, 300)
        # LSTM Cell State Layer 1 - (self.gpu_prediction_batch_size, 300)
        # LSTM Hidden State Layer 2 - (self.gpu_prediction_batch_size, 300)
        # LSTM Cell State Layer 2 - (self.gpu_prediction_batch_size, 300)
        # LSTM Hidden State Layer 3 - (self.gpu_prediction_batch_size, 300)
        # LSTM Cell State Layer 3 - (self.gpu_prediction_batch_size, 300)
        # ]
        ...
        for i in range(len(pwd_list)): # Total number of passwords == total number of output prob distributions
            ...
            next_states.append(
                {
                    "1": (predictions[1][i].reshape(1, self.lstm_units), predictions[2][i].reshape(1, self.lstm_units)),
                    "2": (predictions[3][i].reshape(1, self.lstm_units), predictions[4][i].reshape(1, self.lstm_units)),
                    "3": (predictions[5][i].reshape(1, self.lstm_units), predictions[6][i].reshape(1, self.lstm_units))
                }
            )
    ...
    def compute_states_and_prob(self, compute_string):
        ...
        for index, char in enumerate(compute_string):
            ...
            state = {
                "1" : (prediction[1][0].reshape(1, self.lstm_units), prediction[2][0].reshape(1, self.lstm_units)),
                "2" : (prediction[3][0].reshape(1, self.lstm_units), prediction[4][0].reshape(1, self.lstm_units)),
                "3" : (prediction[5][0].reshape(1, self.lstm_units), prediction[6][0].reshape(1, self.lstm_units))
            }
    ...
    def test_check(self):
        if((self.pwd_generated % self.testing_checkpoint_guesses) == 0):
            # Perform Checkpointing here.
            self.create_pwd_checkpoint()
            # Increment the current checkpoint number
            self.checkpoint_number += 1
            return
        else:
            # Don't need to perform a checkpoint.
            return

    def create_pwd_checkpoint(self):
        # Get the current IO Handle and seek to the 
        # top of the file and then read.
        self.file_handle.seek(0, 0)

        # Can be a large file
        print(f"[+] Creating checkpoint file - {self.checkpoint_number}")
        print(f"[+] Total number of passwords generated - {self.checkpoint_number * self.testing_checkpoint_guesses}")
        with open("testing_checkpoint_" + str(self.checkpoint_number) + ".txt", "w") as ckpt_file:
            for pwd_and_prob in self.file_handle:
                ckpt_file.write(pwd_and_prob)
        print(f"[+] Checkpoint {self.checkpoint_number} created.")
        return
...
if __name__ == "__main__":
    inference_model = tf.keras.models.load_model(".\\H5-Inference-Checkpoint-050.h5")
    tokenizer = loadTokenizer("prototypeTokenizer.json")
    pwd_guesser = Guesser(INFERENCE_MODEL = inference_model, OP_FILE = "password_guesses_lstm_testing.txt", VOCAB_SIZE = 70,
                        LSTM_UNITS = 300, PREDICTION_BATCH_SIZE = 32, RECUR_BATCH_SIZE = 32, TOKENIZER = tokenizer,
                        END_IDX = 2, NUM_GUESS = 15, START = "", PROB = 1.0, MAX_LENGTH = 32, PROB_THRESHOLD = 1e-9,
                        PASSWORD_END_CHAR = "\n", CALCULATE_PROB = False, USE_LENGTH_NORMALIZATION = True,
                        NORMALIZED_SCORE_THRESHOLD = -8.45, NORMALIZATION_ALPHA = 0.37, UNLIMITED_GENERATION = True,
                        TESTING_MODE = True, TESTING_CHECKPOINT = 1e7)
