...
generated_file = ".\\generated_file.txt"
original_file = ".\\original_file.txt"
csv_file_name = ".\\result.csv"

make_data_point_threshold = 2
# 1000 will result in 10^6 + 1 data points for 1 Billion passwords
...
with open(original_file, "r") as og_file:
    ...
    for count, pwd in enumerate(og_file):
        list_pwds.append(pwd.rstrip().lstrip())
    end_time = time.time()
...
def create_data_point(temp_list):
    ...
    matched_list = list(matched_unique)
    return matched_list
